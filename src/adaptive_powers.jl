...
function initialize_noise(weights, scores, count, noise)
    none = Power(
        "additive",
        0.0,
        Dict(:early => 1.0, :mid => 1.0, :late => 1.0),
        copy(scores),
        copy(count),
    )
    low = Power(
...
function initialize_powers(param)
    ...
    for insertion in param[:insertions]
        if insertion == "cheapest"
            push!(
                insertionpowers,
                Power(insertion, 0.0, copy(weights), copy(scores), copy(count)),
            )
        else
            for value in param[:insertion_powers]
                push!(
                    insertionpowers,
                    Power(insertion, value, copy(weights), copy(scores), copy(count)),
                )
            end
        end
    end
    ...
    for removal in param[:removals]
        if removal == "segment"
            push!(
                removalpowers,
                Power(removal, 0.0, copy(weights), copy(scores), copy(count)),
            )
        else
            for value in param[:removal_powers]
                ...
                push!(
                    removalpowers,
                    Power(removal, value, copy(weights), copy(scores), copy(count)),
                )
            end
        end
    end
...
"""
function power_select(RNGs::Array{MersenneTwister, 1}, powers, total_weight, phase::Symbol)
    selection = rand(RNGs[Threads.threadid()]) * total_weight[phase]
    for i in 1:length(powers)
...
function power_weight_update!(
    ...
    for power in powers
        if power.count[phase] > 0 && param[:cold_trials] > 0  # average after 2nd trial
            power.weight[phase] =
                param[:epsilon] * power.scores[phase] / power.count[phase] +
